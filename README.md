...
> 這個服務混有多個來源的 Thing，而且**測站的 authority 與命名並不一致**：
> 多數站標「環境部」、名稱像「空氣品質測站-新莊」，但板橋、土城等站不是。
> 照官方 API 說明頁的範例過濾（authority + 名稱關鍵字）只拿得到 66 站，會漏掉板橋。
> 所以本頁**完全不在伺服器端過濾**，整份清單抓回來後在本機分縣市呈現，
> 打字查詢也會比對名稱與所有 `properties` 欄位。清單上方的下拉選單可依 authority 縮小。
