...
⚠️ 微型感測器與 STA 這兩個來源**只保留最近幾小時**的觀測值（實測約 2 小時）；
要看跨日資料，國家測站請用「環境部開放資料」那個分頁（約 4 天），
微型感測器則仍受限，詳見下方〈只保留最近幾小時的資料〉。
...
> 全部 77 站的資料在環境部自己的開放資料平臺（data.gov.tw 資料集 40448 / `aqx_p_432`），
> 但那是另一支需要 api_key 的 API，本頁尚未整合。
