...
回應欄位與時間格式待覆核。「兩測點差異」面板（原名「來源間偏差」）仍有已知的方法學問題，
**在「下一輪必修」處理完之前，那個面板的數字不得用於任何結論**
（2026-09-05 已改名為「兩測點差異」並補上距離警告與限制說明，但 CI／LoA／BA 的 x 軸還沒修）。

**最重要的現實限制：三個 API 都只保留近期觀測值**（微型感測器實測約 **4～5 小時**——
舊版寫的 2 小時已被推翻，見「已驗證的事實」；環境部開放資料約 4 天）。「往前天數」選再多，**API 那一段**也拿不到更早的資料
——這不是程式的 bug，是資料來源的設計。
...
**⚠️ 環境部開放資料 `aqx_p_488` 疑似大幅落後（2026-09-06，只有使用者畫面上的觀察，
我沒有自己驗證過）**：使用者查 7 天（09-01～09-07）時，板橋只回了 **39 小時、
而且停在 09-02 14:00 左右**——比「約 4 天」的舊量測短，而且最新一筆落後當下約 5 天。
`loadMoenvStation()` 是用 `sort=datacreationdate desc` 加 `limit=216` 請求的，
拿回 39 筆代表**該站在這支資料集裡就只有那 39 筆**，不是被 limit 截斷。
兩種可能還沒分開：資料集本身發布落後，或 `sort` 參數沒有生效。
**要判斷就按頁面上的「診斷」**，那裡有 `aqx_p_488` 有排序／不排序兩條對照，
看兩者最新一筆的時間就知道。在確認之前，不要把「約 4 天」當成現況。
...
- **已經錯過的時段永久取不回來**——API 只留 4～5 小時，沒有第二條路。

`api-append` 因此縮短到**每 30 分鐘**當保險，但**實際損失比一開始估的小很多**
（見下方「排程漏跑造成的損失只有約 5%」）。
`hist-daily` 仍保留（每天 8 個小請求），站方若恢復產出就會自動接回來。
**未向官方求證**，只有這次探測的觀測證據；先前查到的「資料服務只提供到 2026-12-01」
看來提早發生，或至少日檔這一塊已經停了。

**排程漏跑造成的損失只有約 5%（2026-09-04 實測，別再高估）**：以 09-03 一整天
（1272／1440 筆，88.3%）拆解相鄰時標的間隔——
零星的 2～4 分小跳點共 79 處、99 分鐘（**感測器／API 自己就沒有每分鐘回報**），
真正的大缺口只有 2 個（16 分 ＋ 53 分）共 69 分鐘。
**排程可歸因的損失是 69/1440 ＝ 4.8%**，其餘 6.9% 修不掉。
配合 4～5 小時的保留期，實測平均 3.2 小時的觸發間隔大多接得上。
所以「漏跑一次就永久少一大段」是**過度悲觀**的說法，不要拿它當改架構的理由；
要評估就照這個方法重算一次缺口分布。
...
不是感測器離線。對策見「程式碼慣例」裡的 schedule 與 `--skip-done` 兩條。
...
| **資料保留期** | **兩個 STA endpoint 都只保留近期觀測值。** 微型感測器實測**約 4～5 小時**（不是舊版寫的 2 小時）。2026-09-04 以五份 `$top=500` 的 API 快照量測：跨度 4.9／4.9／3.9／4.2／4.0 小時，且**最舊一筆都剛好落在整點**，看起來是「保留最近 4～5 個整小時」。舊版「約 2 小時／`@iot.count`＝121」是 2026-08-18 的單次量測，已被推翻——**不要再引用那個數字**。查再長的區間仍然拿不到更早的資料 |
...
4. **國家測站的資料保留期** — 微型感測器已實測約 4～5 小時（見「已驗證」）。
   國家測站是小時值，如果保留期也是幾小時，那**一次只拿得到幾個點**，
...
## 下一輪必修：「兩測點差異」面板的方法學問題

現行 `renderBias()`／`blandAltman()`／`rhStrata()` 有以下問題，**在第 2、5、6 項修好之前，
這個面板的數字不得用於任何結論，也不得寫進報告**（面板上現在自己也寫著這句）。
使用者（醫學／流行病學背景）已逐項指出。**1、3、4 已於 2026-09-05 完成**：

1. ~~**非 co-location，不能叫 bias。**~~ — **2026-09-05 已做。** 面板標題、表頭、
   Bland–Altman 圖的標線、濕度分層表頭與結語文案全部改成「兩測點差異」／「平均差值」，
   畫面上不再出現 `bias`。載入後面板頂端會依 `haversineKm()` 算出的距離顯示警告：
   ≥ 2 km 明寫「不是並置（co-location）」「不能當成儀器偏差，也不可作為校正依據」，
   ≥ 30 km 再加一句「可能分屬不同空品區」。實測這一對顯示 **34.1 km**。
   `#bias`／`#biasMeta`／`renderBias()` 這些 **id 與函式名維持不變**（既有選擇器與測試會用到），
   只有畫面上的字改掉。原始描述：目前這一對（`13580653094` ↔ 板橋）
   **相距約 34 km**，分屬桃園沿海與新北市區，連「同一個空氣環境」都稱不上。
   兩者的差值同時混合了「儀器／方法差異」與「真實的空間濃度梯度」，兩者無法用現有資料拆開。
   若日後真的做 co-location（兩機並置），才可以重新談儀器偏差。

2. **95% CI 低估。**（**未做**）逐時 PM2.5 有強烈自相關，現行 `mean ± 1.96·SD/√n` 把 n 當成獨立樣本數，
   信賴區間必然過窄。→ 改以**「日」為 block 的 block bootstrap**（重抽整天，保留日內相關結構）
   估計差異平均值的 CI；報告時說明重抽次數與 block 定義。
   目前只在面板上**明講這個 CI 偏窄**，數字本身還是舊的算法。

3. ~~**樣本量門檻改以天數計。**~~ — **2026-09-05 已做。** 面板會顯示「配對涵蓋 N 天／M 個小時」，
   並說明有效獨立樣本接近天數；`dayCount < 7` 或 `st.n < 24` 就標示證據薄弱。

4. ~~**Pearson r 不是一致性指標。**~~ — **2026-09-05 已做（採「保留但標註」那個選項）**：
   表頭加 `※`，下方註明「r 高不代表兩者的值可以互換，只代表一起漲跌；要看一致性請看一致性界限」。
   沒有直接移除，是因為使用者要判斷「兩站的時間結構有沒有同步」時 r 仍然是有用的敘述統計——
   但它**不得**被當成一致性證據。若日後覺得還是會誤導，直接刪掉那一欄即可。

5. **固定 ±1.96SD 的 LoA 可能不適用。**（**未做**）若差值的離散度隨濃度放大（heteroscedasticity，PM2.5 常見），
   固定寬度的一致性界限在低濃度過寬、高濃度過窄。
   → 先檢查差值 vs 平均值的散布是否有喇叭狀；若有，改用 **log 轉換後的 LoA**（結果以比值呈現）
   或 **回歸式 LoA**（差值與 SD 皆對濃度做迴歸）。UI 要標明用的是哪一種。

6. **確認 Bland–Altman 的 x 軸。**（**未做**）現行 `blandAltman()` 的 x 軸**確實是 (微型 + 國家)/2**
...
**做這一輪時請一併重讀原則 3。** 面板已改名，README 與 UI 文案已同步（2026-09-05）。
第 2、5、6 項還沒做，面板底部有一段橘色警告逐項列出，修完要記得把那段一起拿掉。
...
- **「200 ＋ HTML」對不存在的「日期」同樣成立，而且會讓整個 workflow 掛掉**
  （2026-09-02／03／04 三次 `hist-daily` 都紅在這裡）。2026-09-01 的日檔站方根本沒有，
  卻回了 200 加一個 **0 MB** 的頁面 → `download()` 順利完成 → `zipfile` 以
  `BadZipFile` 未捕捉例外中止，**離開碼 1，後面的日子與另一個測項全都沒跑到**。
  現在 `check_zip()` 在解壓前先驗 ZIP 的魔術位元組 `PK\x03\x04`，不是就當成
  「這一天站方沒有」：`--skip-missing` 下跳過並在結尾印 `::warning::` 列出是哪幾天，
  沒有 `--skip-missing` 就以清楚的訊息失敗（不是丟一個 `BadZipFile` traceback）。
  **下載成功 ≠ 拿到檔案**，這條對這個來源要一直記著。
...
- **「今天」的缺口是結構性的**：history 日檔隔天 03:00 才有（**且 2026-08-31 後已停更**）、
  API 只留 4～5 小時。`api-append.yml` 定期補一次把它縮小。
  **間隔不可以拉長超過保留期**（目前實測 4～5 小時，現行設定 30 分鐘，餘裕充足）。
  舊版寫「不可超過 2 小時」是根據被推翻的保留期數字，別再照抄。
- **GitHub 的 `schedule` 不保證會觸發，這是資料缺口的主因之一**（2026-08-28 實測）：
  ...
  - `api-append` 當時設每 2 小時（一天應有 12 次），2026-08-26T22:47Z 之後到
    08-28T04:04Z 之間只跑了 **3 次**，間隔 9～11 小時。
    （當時以為保留期是 2 小時；2026-09-04 更正為 4～5 小時，所以那次的實際損失
    比當時估的小，但 9～11 小時的間隔仍然會掉資料。）
...
- **`--skip-done` 讓「拉寬視窗」不等於「多下載」，而且判準是帳本不是列數**：
  `data/_hist_days.json` 記下每個（裝置 × 測項 × 台灣日期）**有沒有真的抓過那個日檔**，
  抓過就整天跳過、不下載那 220 MB。穩定狀態下每天只下載「昨天」一個檔。
  **不要退回用列數當門檻**（原本是 `--skip-complete 1200`，已移除）：
  `api-append` 改成每小時之後，它自己就能在一天內累積到一千多列
  （實測 2026-09-02 有 1230 列、09-03 有 1272 列，兩天都不滿 1440，而且 24 小時都有格），
  那些日子會被永久判成「完整」而跳過，history 的權威日檔再也不會被抓，
  那一天就固定殘缺下去。**「有沒有抓過」是事實，不該用列數去推論。**
  抓到了但該裝置在檔案裡沒有資料，一樣記帳——重抓不會變出資料來。
  只有「根本沒下載到」（連不到、或回 200＋HTML）才不記，下次會再試。
  要強制重抓就把那個日期從 `_hist_days.json` 刪掉。
  帳本是 2026-09-04 用 `--seed-done 1300` 一次性搬遷建立的（當時純 api-append 的
  日子最多 1272 列，完整日最少 1333 列，門檻取 1300 能把兩群分開）——
  **那個列數判準只用於搬遷那一次，之後不再使用**。
  **注意**：全部日子都被跳過、或站方就是沒有那些檔案時，`total_rows` 會是 0，
  那是正確結果不是故障；離開碼 4 只在「沒跳過、也沒缺檔、卻一列都沒寫入」時才觸發。
...
- **「回溯不夠久」與「更新落後」是兩件不同的事，兩邊都要檢查**：原本只比對
  `st.range.oldest > startUTC`（來源回溯不到區間開頭），**完全沒有檢查最新的一筆**。
  2026-09-06 使用者回報：板橋那條線停在 09-02、右邊五天整片空白，而狀態列一個字都沒提。
  現在載入結束後會逐站算「最後一筆」與 `endUTC` 的差，超過 3 小時就明講是
  **來源沒有更新到那麼近**，並重申不是抓取失敗、也不是感測器離線。
  歸因的四種原因（保留期不足／來源更新落後／抓取失敗／感測器離線）**訊息一律不可混用**。
- **這一頁沒有 markdown 轉換，`**粗體**` 會原樣印出來**：`setStatus()` 與大部分面板都是
  直接 `innerHTML`，只有 `renderProv()` 那一處對 `distanceNote()` 的回傳值做了
  `replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>")`。2026-09-06 查出三處把 `**` 當粗體寫
  （加入測站的 hint、診斷區說明、小時統計表說明），畫面上就是兩顆星星——
  其中一處還在同一個字串裡混用了 `<strong>`。**要粗體就寫 `<strong>`。**
- **「歷史檔 A ＋ API B」不等於總筆數，不要那樣寫**：(a) 重疊的分鐘會去重，A+B 比合併後多；
  (b) `histCount`／`apiCount` 只存在於微型感測器那幾條序列，國家測站的點不在那兩個桶子裡。
  使用者實際看到 `6663 ＋ 381` 對不上 `6988`。現在寫成
  「其中微型感測器 C 筆＝repo 歷史檔 A ＋ API 即時 B − 重疊 D」，算式自己要對得起來。
- **沒有歷史檔的裝置，不可以只叫使用者「跑回填 workflow」**：history 站自 2026-08-31 起
  不再產出日檔，回填只補得到 08-31 以前。使用者看的多半是最近幾天，照那句話做會白跑一次
  workflow 然後發現圖上什麼都沒多。訊息要把兩條路分開講：08-31 以前用回填；
  09-01 以後只能把編號加進 `api-append.yml` 的 `DEVICES` 從當下開始累積，過去的補不回來。
...
- **統計一律以小時平均為單位**（`hourlyMap()`）：兩個來源時間解析度不同，先降頻成台灣時間整點才可比。
  ...
  分層與兩測點差異面板的 `n` 一律是「有效小時數」，不是原始筆數，UI 不得寫成筆數。
...
1. **history.colife.org.tw → GitHub Actions 預處理**（**主體已完成**，只剩時區的決定性驗證，
   見「未驗證」第 6 項） — STA 只留幾小時（微型實測 4～5 小時），這條沒做的話，
   ...
   **階段 C（前端合併，已做）**：網頁以同源方式讀 `data/manifest.json` 與
   `data/iot/<裝置>/<YYYY-MM>.csv`，補在 API 那幾小時的前面。
...
2. **修「兩測點差異」面板剩下的方法學問題** — 見上方「下一輪必修」第 2、5、6 項（1、3、4 已於 2026-09-05 完成）。
...
4. ~~取得國家空品測站 endpoint~~ — 已完成（colife 那台）。
   ...
   ~~釐清 7 天問題~~ — 已完成（成因 C：保留期只有幾小時；當時記為 2 小時，2026-09-04 更正為 4～5 小時）。
...
6. **長期資料累積（history 來源已停更，這是唯一的累積管道；但別為了「漏跑」而搬家）**：
   2026-09-01 起 `api-append` 是唯一來源，每天約 1200～1300 列（88%）。
   **其中排程可歸因的損失只有約 5%**，其餘是感測器／API 自己的跳點（見「現況」段落的實測拆解）。
   搬到別的排程器最多只能救回那 5%，成本效益要自己衡量；
   這一項真正的價值在於**資料落地後不會被來源清掉**，不是為了補那 5%。
   因為 STA 只留幾小時，這一項的價值比原本高得多——
