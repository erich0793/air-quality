...
3. **缺的 11 站：環境部開放資料平臺（部分已實測）** — `data.moenv.gov.tw`（data.gov.tw 資料集 40448）
   ...
   - **仍待確認**：`aqx_p_488` 是否真的給時間序列（幾筆、涵蓋多久、間隔是否為 1 小時）、
     時間欄位名稱與時區、以及是否強制要求 api_key。BUILD `2026-08-19h` 的探測會回答這些。
