...
3. **缺的 11 站要不要另尋來源** — 環境部自己的開放資料平臺（`data.moenv.gov.tw` 的
   `aqx_p_432`，即 data.gov.tw 資料集 40448）有全部 77 站含板橋，但那是**另一個平台的另一支 API**：
   需要註冊 api_key、每日 5,000 次上限、回的是**最新一小時的快照**而非時間序列，CORS 未測。
   要不要接進來是待決策事項，見待辦。
