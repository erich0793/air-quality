...
3. **平日 vs 假日、尖峰 vs 離峰的分層統計**，含樣本數與離散度，不要只給平均值。
4. **微型 vs 國家測站的偏差量化**：把微型感測器降頻成小時平均後與國家測站配對，
   算 bias、MAE、相關係數與 Bland–Altman，並依濕度分層（濕度是已知的干擾因子）。
   注意：降頻成小時平均是統計處理，不是內插，可以做；但仍不得用來「校正」原始曲線。
